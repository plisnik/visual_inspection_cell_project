import cv2
import numpy as np
import os
import sys
from typing import List
sys.path.append(os.path.join(os.path.dirname(__file__), '..'))
from utils import utilities

# EYE-TO-HAND: CAMERA OUTSIDE THE ROBOT
# Define path to the dataset folder
data_set = "data_sets\dataset_20250328_094912"

obj_pose_folder = os.path.join(data_set, 'obj_pose_tf')  # Path to the folder containing object poses
robot_pose_folder = os.path.join(data_set, 'robot_pose_tf')  # Path to the folder containing robot poses
rob_pose_tf_list: List[np.ndarray] = utilities.load_npy_data(robot_pose_folder)
obj_pose_tf_list: List[np.ndarray] = utilities.load_npy_data(obj_pose_folder)

# Invert the robot pose transformation matrices and decompose them
inv_rob_pose_tf_list: List[np.ndarray] = utilities.invert_tf_matrices(rob_pose_tf_list)
R_base2grip, t_base2grip = utilities.decompose_tf_matrices(inv_rob_pose_tf_list)
# Decompose the transformation matrices into rotation and translation components
R_target2cam, t_target2cam = utilities.decompose_tf_matrices(obj_pose_tf_list)
inv_obj_pose_tf_list: List[np.ndarray] = utilities.invert_tf_matrices(obj_pose_tf_list)
R_cam2target, t_cam2target = utilities.decompose_tf_matrices(inv_obj_pose_tf_list)

# Perform Hand-Eye calibration - Eye-to-Hand
R_cam2base, t_cam2base = cv2.calibrateHandEye(
   R_base2grip, t_base2grip,
   R_target2cam, t_target2cam,
   method=cv2.CALIB_HAND_EYE_ANDREFF  # You can choose another method, e.g., 'CALIB_HAND_EYE_PARK', etc.
)

# Construct the resulting transformation matrix X (camera to robot base)
T_cam2base: np.ndarray = np.eye(4)
T_cam2base[:3, :3] = np.array(R_cam2base)
T_cam2base[:3, 3] = np.array(t_cam2base).flatten()

print("Resulting transformation matrix X (camera to robot base) ANDREFF:")
print(T_cam2base)

# Perform Hand-Eye calibration - Eye-to-Hand
R_cam2base, t_cam2base = cv2.calibrateHandEye(
   R_base2grip, t_base2grip,
   R_target2cam, t_target2cam,
   method=cv2.CALIB_HAND_EYE_DANIILIDIS  # You can choose another method, e.g., 'CALIB_HAND_EYE_PARK', etc.
)

# Construct the resulting transformation matrix X (camera to robot base)
T_cam2base = np.eye(4)
T_cam2base[:3, :3] = np.array(R_cam2base)
T_cam2base[:3, 3] = np.array(t_cam2base).flatten()

print("Resulting transformation matrix X (camera to robot base) DANIILIDIS:")
print(T_cam2base)

# Perform Hand-Eye calibration - Eye-to-Hand
R_cam2base, t_cam2base = cv2.calibrateHandEye(
   R_base2grip, t_base2grip,
   R_target2cam, t_target2cam,
   method=cv2.CALIB_HAND_EYE_HORAUD  # You can choose another method, e.g., 'CALIB_HAND_EYE_PARK', etc.
)

# Construct the resulting transformation matrix X (camera to robot base)
T_cam2base = np.eye(4)
T_cam2base[:3, :3] = np.array(R_cam2base)
T_cam2base[:3, 3] = np.array(t_cam2base).flatten()

print("Resulting transformation matrix X (camera to robot base) HORAUD:")
print(T_cam2base)

# Perform Hand-Eye calibration - Eye-to-Hand
R_cam2base, t_cam2base = cv2.calibrateHandEye(
   R_base2grip, t_base2grip,
   R_target2cam, t_target2cam,
   method=cv2.CALIB_HAND_EYE_PARK  # You can choose another method, e.g., 'CALIB_HAND_EYE_PARK', etc.
)

# Construct the resulting transformation matrix X (camera to robot base)
T_cam2base = np.eye(4)
T_cam2base[:3, :3] = np.array(R_cam2base)
T_cam2base[:3, 3] = np.array(t_cam2base).flatten()

print("Resulting transformation matrix X (camera to robot base) PARK:")
print(T_cam2base)

# Perform Hand-Eye calibration - Eye-to-Hand
R_cam2base, t_cam2base = cv2.calibrateHandEye(
   R_base2grip, t_base2grip,
   R_target2cam, t_target2cam,
   method=cv2.CALIB_HAND_EYE_TSAI  # You can choose another method, e.g., 'CALIB_HAND_EYE_PARK', etc.
)

# Construct the resulting transformation matrix X (camera to robot base)
T_cam2base = np.eye(4)
T_cam2base[:3, :3] = np.array(R_cam2base)
T_cam2base[:3, 3] = np.array(t_cam2base).flatten()

print("Resulting transformation matrix X (camera to robot base) TSAI:")
print(T_cam2base)

# Perform Robot-World/Hand-Eye calibration - Eye-to-Hand
R_base2cam_W, t_base2cam_W, R_grip2target_W, t_grip2target_W = cv2.calibrateRobotWorldHandEye(
   R_cam2target, t_cam2target,
   R_base2grip, t_base2grip,
   method=cv2.CALIB_ROBOT_WORLD_HAND_EYE_SHAH  # You can choose another method, e.g., 'CALIB_ROBOT_WORLD_HAND_EYE_SHAH', etc.
)

# Construct the resulting transformation matrix X (camera to robot base)
T_base2cam_W: np.ndarray = np.eye(4)
T_base2cam_W[:3, :3] = np.array(R_base2cam_W)
T_base2cam_W[:3, 3] = np.array(t_base2cam_W).flatten()

print("Resulting transformation matrix X (camera to robot base) SHAH:")
T_cam2base_W: np.ndarray = np.linalg.inv(T_base2cam_W)
print(T_cam2base_W)

# Perform Robot-World/Hand-Eye calibration - Eye-to-Hand
R_base2cam_W, t_base2cam_W, R_grip2target_W, t_grip2target_W = cv2.calibrateRobotWorldHandEye(
   R_cam2target, t_cam2target,
   R_base2grip, t_base2grip,
   method=cv2.CALIB_ROBOT_WORLD_HAND_EYE_LI  # You can choose another method, e.g., 'CALIB_ROBOT_WORLD_HAND_EYE_SHAH', etc.
)

# Construct the resulting transformation matrix X (camera to robot base)
T_base2cam_W = np.eye(4)
T_base2cam_W[:3, :3] = np.array(R_base2cam_W)
T_base2cam_W[:3, 3] = np.array(t_base2cam_W).flatten()

print("Resulting transformation matrix X (camera to robot base) LI:")
T_cam2base_W = np.linalg.inv(T_base2cam_W)
print(T_cam2base_W)