...
class RobotInterface:
    """
    Robot interface supporting both RTDE and PLC OPC UA communication modes.
    """
    
    def __init__(self, ip_address: str, mode: str = "rtde") -> None:
        """Initialize robot interface with specified communication mode."""
        self.ip = ip_address
        self.mode = mode

        if self.mode not in ["rtde", "plc_opcua"]:
            raise ValueError(f"Unknown robot control mode: {mode}")
    ...
    def moveL(self, pose: List[float], speed: float = 0.25, acceleration: float = 0.25) -> bool:
        """Move robot in linear motion to specified pose."""
        if self.mode == "rtde":
    ...
    def moveJ(self, pose: List[float], speed: float = 0.25, acceleration: float = 0.25) -> bool:
        """Move robot in joint motion to specified pose."""
        if self.mode == "rtde":
            rtde_c = rtde_control.RTDEControlInterface(self.ip)
    ...
    def setStandardDigitalOutput(self, output_id: int, bool_value: bool) -> Optional[bool]:
        """Set digital output state."""
        if self.mode == "rtde":
            rtde_IO = rtde_io.RTDEIOInterface(self.ip)
            success = rtde_IO.setStandardDigitalOut(output_id, bool_value)
            time.sleep(0.1)
            rtde_IO.disconnect()
            return success
        elif self.mode == "plc_opcua":
            if output_id == 0:
                PLC_comunication.write_value_bool(self.client,'ns=3;s="FB_DB".UserDataInput."Bool".Register[0]', bool_value)
    ...
    def getStandardDigitalOutput(self, output_id: int) -> Optional[bool]:
    ...
    def isConnected(self) -> Optional[bool]:
        ...
        if self.mode == "rtde":
            ...
            PLC_comunication.read_value_bool(self.client,'ns=3;s="URI".State.Robot."PW: Is power on"')  # check if robot is connected and powered on
            return None
        
    def getRobotStatus(self) -> Optional[int]:
        """Get robot status."""
        if self.mode == "rtde":
            rtde_c = rtde_control.RTDEControlInterface(self.ip)
            status = rtde_c.getRobotStatus()
